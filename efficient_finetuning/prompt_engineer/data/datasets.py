...
class Flowers102(ClipExptDataset):
    ...
    def get_all_data_loader(self, transform_fn=None, shuffle=True):

        if transform_fn is None:
            transform_fn = self.train_transform

        dataset = datasets.ImageFolder(
            self.root + "flower_data/all", transform=transform_fn
        )
...
#############################################################
# TODO: Please refactor this to support split by class label
#############################################################

class Flowers102FSLTrain(ClipExptDataset):

    def __init__(self, num_workers, batch_size, root=None):
        super().__init__(num_workers, batch_size, root)
        self.name = "Flowers102FSLTrain"
        self.class_label_mapping = {
            "pink primrose": 0,
            "globe thistle": 1,
            "blanket flower": 2,
        ...
        self.classes = [str(x) for x in list(self.class_label_mapping.keys())]
        self.new_labels = {v:i for i,v in enumerate(list(self.class_label_mapping.values()))}

    def get_all_data_loader(self, transform_fn=None):
        ...
        small_train_dataset = torch.utils.data.Subset(train_dataset, train_indices)
        small_train_dataset.targets = np.array(train_dataset.targets)[train_indices]
        small_train_dataset.target_transform = target_transform
        ...
        small_train_dataset.classes = self.classes

        return small_train_dataset, loader

class Flowers102FSLTest(ClipExptDataset):

    def __init__(self, num_workers, batch_size, root=None):
        super().__init__(num_workers, batch_size, root)
        self.name = "Flowers102FSLTest"
        self.class_label_mapping = {
            "canna lily": 92,
            "hippeastrum": 93,
            "bee balm": 94,
    ...
    def get_all_data_loader(self, transform_fn=None):
        ...
        indices = np.argwhere(np.isin(np.array(dataset.targets),list(self.class_label_mapping.values()))).flatten()
        
        small_dataset = torch.utils.data.Subset(dataset, indices)
        small_dataset.targets = np.array(dataset.targets)[indices]

        small_dataset.target_transform = target_transform
        ...
        small_dataset.classes = self.classes

        return small_dataset, loader