    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(tqdm(test_loader)):
            ...
            for image_feature, image_feature_norm ,tar in zip(image_features, image_features_norm, target):
                ...
                # print("###################################")
                # print(a,b)
                # np.savetxt('debug.txt', (linear_probe).cpu().numpy())
                # break
...
def weighted_image_and_text_embedding(train_features, train_labels, test_loader, classes):
    ...
    for lam in lams:
        print("---------- Lam:", lam, "----------")
        res = image_and_text_embedding_1(train_features, train_labels, test_loader, classes, a=lam,b=1-lam)
        out[lam] = res
    return out 
...
ALL_EXPTS = [weighted_image_and_text_embedding_0_5]
...
def run_expts(args):
    ...
    # with open(f"{results_path}/{dataset}/all_minus_1.json", "w") as outfile:
