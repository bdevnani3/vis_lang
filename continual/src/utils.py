...
def some_sanity_checks(args):
    ...
    if not args.ntasks in datasets_tasks[args.data]:
        raise Exception(
            "Chosen number of tasks ({}) does not match with {} experiment".format(
                args.ntasks, args.experiment
            )
        )


def save_code(checkpoint):
    cwd = os.getcwd()
    des = os.path.join(checkpoint, "code") + "/"
