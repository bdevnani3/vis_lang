...
class DatasetGen(object):
    ...
    def __init__(self, args):
        ...
        self.taskcla = [
            [t, int(self.num_classes / self.num_tasks)] for t in range(self.num_tasks)
        ]
    ...
    def get(self, task_id):
        ...
        self.train_set[task_id] = iCIFAR100(
            root=self.root,
            classes=self.task_ids[task_id],
            task_num=task_id,
            train=True,
            download=True,
            transform=self.transformation,
        )
        self.test_set[task_id] = iCIFAR100(
            root=self.root,
            classes=self.task_ids[task_id],
            task_num=task_id,
            train=False,
        ...
        train_split, valid_split = torch.utils.data.random_split(
            self.train_set[task_id], [len(self.train_set[task_id]) - split, split]
        )
        ...
        print(
            "Training set size:   {} images of {}x{}".format(
                len(train_loader.dataset), self.inputsize[1], self.inputsize[1]
            )
        )
        print(
            "Validation set size: {} images of {}x{}".format(
                len(valid_loader.dataset), self.inputsize[1], self.inputsize[1]
            )
        )
        print(
            "Train+Val  set size: {} images of {}x{}".format(
        ...
        print(
            "Test set size:       {} images of {}x{}".format(
                len(test_loader.dataset), self.inputsize[1], self.inputsize[1]
            )
        )
