...
def load_data_cifar10(train=True):
    ...
    dataset = datasets.CIFAR10(root=os.path.join(ROOT_PATH, 'data'), train=train,
...
def init_word_lookup(model):
    ...
    model.word_lookup = torch.from_numpy(np.stack([word_vectors[_class] for _class in class_names])).to(get_device())
...
def find_closest_words(word_lookup:torch.Tensor, x:torch.Tensor, mode:str='l2') -> torch.Tensor:
    """
    ...
    if mode == 'l2':
        return ((word_lookup[None, :, :] - x[:, None, :]) ** 2).sum(dim=-1).argmin(dim=-1)
    else:
...
def train_model(epoch_idx, model):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        total += labels.size(0)
        correct += (find_closest_words(model.word_lookup, outputs) == labels).sum().item()
    ...
    print(f'Epoch {epoch_idx} || Loss: {epoch_loss:7.3f} | Accuracy: {epoch_accuracy:6.2f}% || Time: {elapsed:6.2f}s')
...
def validate_model(epoch):
    ...
    if best_accuracy < epoch_accuracy:
        ...
        print("Saving model with acc: {:6.2f}%, loss: {:.3f}, epoch: {}".format(epoch_accuracy, epoch_loss, epoch))
        best_acc_path = os.path.join(ROOT_PATH, 'trained_models/vis_lang/cifar10_emb_best_acc.pth')
    ...
    if min_loss > epoch_loss:
        min_loss = epoch_loss
        print("Saving model with acc: {:6.2f}%, loss: {:.3f}, epoch: {}".format(epoch_accuracy, epoch_loss, epoch))
        best_loss_path = os.path.join(ROOT_PATH, 'trained_models/vis_lang/cifar10_emb_best_loss.pth')
